...
class StatisticClient
{
     ...
     * @return void
     */
    public function tick($module = '', $interface = '')
    {
     ...
     * @return boolean
     */
    public function report($module, $interface, $success, $code, $msg, $report_address = '')
    {
        $report_address = $report_address ? $report_address : 'udp://localhost:30000';
...
class StatisticProtocol
{
    ...
    public static function encode($module, $interface , $cost_time, $success,  $code = 0,$msg = '')
    {
        ...
        var_dump($module_name_length);
        var_dump($interface_name_length);
        var_dump($cost_time);
        $a =  $success ? 1 : 0;
        var_dump($a);
        var_dump($code);
        ...
        var_dump(time());
        // 打包
       return  pack('CCfCNnN', $module_name_length, $interface_name_length, $cost_time, 
        $success ? 1 : 0, $code, strlen($msg), time()).$module.$interface.$msg;
    }
...
{
    (new StatisticClient())->tick("TestModule", 'TestInterface');
    usleep(rand(10000, 600000));
    $success = rand(0,1);
    $code = rand(300, 400);
    $msg = 'bbbbbbbbbb';
    var_export((new StatisticClient())->report('TestModule', 'TestInterfacedddddd', $success, $code, $msg));;
}
